...
class Redis_cli
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $this->redis_client->connect($_ENV['REDIS_HOST'],$_ENV['REDIS_PORT']);
        } catch (\Exception $e) {
    ...
    public function setList($var, $element)
    {
        try {
            $res = $this->redis_client->sAdd($var, $element);
            return $res;
        } catch (\Exception $e) {
            $this->message['Message'] = $e->getMessage();
            $this->message['Code'] = "500";
    ...
    public function setStruct($key, $field, $value)
    {
        try {
            $res = $this->redis_client->hSet($key, $field, $value);
            $this->$res;
        } catch (\Exception $e) {
            $this->message['Message'] = $e->getMessage();
            $this->message['Code'] = "500";
    ...
    public function getRedisVar($key)
    {
        try {
    ...
    public function getSmembers($variable)
    {
        try {
            $var = $this->redis_client->sMemers($variable);
            if ($var) {
                $this->message['Message'] = "Variable obtenida correctamente";
                $this->message['Code'] = "200";
                $this->message['Data'] = $var;
    ...
    public function variableIsExistAtRedis($key)
    {
        try {

            if ($this->redis_client->exists($key)) {
                $this->message['Message'] = "Variable existente";
                $this->message['Code'] = "200";
            } else {
                $this->message['Message'] = "Variable no encontrada";
                $this->message['Code'] = "404";
